...
get_previous_abstract() {
    if [ -f "$DATA_DIR/summary_latest.md" ]; then
        # 提取摘要部分（从 ## 摘要 到文件结尾）
        sed -n '/^## 摘要/,$p' "$DATA_DIR/summary_latest.md" | tail -n +2 | xargs
    else
        echo "暂无历史摘要"
    fi
}
...
这是一个利用搜索工具，根据用户指定的关键词进行搜索并进行分析筛选出与关键词有强相关性的内容的任务。这里的强相关性不只是内容包含用户指定的关键词，而是在真正理解内容的基础上，通过思考判断其本质是否与用户指定关键词相关联。
...
- **严格过滤无关内容**：以下类型的内容必须过滤掉：
  ...
  - 仅标题包含关键词但内容不相关
  - 广告、推广内容
- **宁缺毋滥**：宁可结果少，也要确保最终结果匹配，如果没有非常高的匹配项，则执行输出无匹配项的流程。
...
2. 内容是否与关键词所描述的核心高度匹配（而非仅作为无关例子出现）？
3. 如果答案是否定的 → **过滤掉**
