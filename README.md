...
定时技术创新监测任务自动化实验项目.
opencode + cron + bash实现的一个极简AI agent定时任务系统。
...
### 仅开发了最核心的部分
因为在定时任务系统中，大模型的参与是最灵活，最不可控的存在。大模型相关的一切都在日新月异地改变，API在变，MCP在变，SKILLS在变……。因此根本没有把握能够设计出能够长期运行的系统，但是我仍然想要测试一下，一个简单的文本读写存储的实现能够走多远，效果到底怎么样。
